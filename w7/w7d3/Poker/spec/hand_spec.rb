...
describe Hand do
    ...
    let!(:new_cards) { [Card.new(:spades, :five), Card.new(:clubs, :three)] }
        ...
        #     # expect(hand.cards).to_not inlcude(*take_cards)
        ...
        it 'raises an error if trade does not result in five card' do
            expect do
        ...
        it 'raises an error if trade tries to discard unowned card' do
            expect do
                hand.trade_cards([Card.new(:hearts, :queen)], new_cards[0..0])
